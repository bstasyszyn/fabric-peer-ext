...
import (
	...
	config2 "github.com/trustbloc/fabric-peer-ext/pkg/config"
...
func New(channelID string, disc *discovery2.Discovery, pp policies.Provider, policyRetriever PolicyRetriever, bp api2.BlockPublisher, validatorDiscovery ValidatorDiscovery) *Policy {
	cfg := &config{
		committerTransactionThreshold:  config2.GetValidationCommitterTransactionThreshold(),
		singlePeerTransactionThreshold: config2.GetValidationSinglePeerTransactionThreshold(),
	}

	evaluator := newEvaluatorWrapper(channelID, newEvaluatorCreator(cfg, disc, pp, policyRetriever, validatorDiscovery))
...
func (v *Policy) GetValidatingPeers(block *cb.Block) (discovery2.PeerGroup, error) {
...
func newEvaluatorCreator(cfg *config, disc *discovery2.Discovery, pp policies.Provider, policyRetriever PolicyRetriever, validatorDiscovery ValidatorDiscovery) func(string) (policyEvaluator, error) {
	return func(channelID string) (policyEvaluator, error) {
		...
		return newEvaluator(channelID, cfg, disc, validator, orgValidator, inquireablePolicy, validatorDiscovery, !exists)
	}
}
