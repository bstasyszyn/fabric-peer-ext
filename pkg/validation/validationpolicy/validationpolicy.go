...
func New(channelID string, disc *discovery2.Discovery, pp policies.Provider, policyRetriever PolicyRetriever) *Policy {
	evaluator := newEvaluatorWrapper(channelID, newEvaluatorCreator(disc, pp, policyRetriever))
...
func newEvaluatorCreator(disc *discovery2.Discovery, pp policies.Provider, policyRetriever PolicyRetriever) func(string) (policyEvaluator, error) {
	return func(channelID string) (policyEvaluator, error) {
		...
		return newEvaluator(channelID, disc, validator, orgValidator, inquireablePolicy, !exists)
	}
}
