...
func newEvaluator(channelID string, cfg *config, discovery *discovery.Discovery, validator, orgValidator policies.Policy,
...
// PeerGroups selects the group of peers for validating a block based on the validation policy.
// The choice of peers is based on several criteria:
...
func (e *evaluator) PeerGroups(block *cb.Block) (peergroup.PeerGroups, error) {
	...
	committers := discovery.PeerGroup(e.Discovery.GetMembers(
		func(member *discovery.Member) bool {
			// This check would need to be removed for cross-org validation
			if member.MSPID != e.Self().MSPID || member.Properties == nil {
				return false
			...
			return roles.FromStrings(member.Properties.Roles...).Contains(roles.CommitterRole)
		},
	))
	...
	if numTransactions < e.committerTransactionThreshold {
		for _, member := range committers {
			selectedValidator = member

			logger.Infof("[%s] Selected validator is %s (committer) since the number of transactions in block %d is %d which is less than the committer threshold %d",
				e.channelID, member.Endpoint, block.Header.Number, numTransactions, e.committerTransactionThreshold)

			break
		}
	}

	if selectedValidator == nil && numTransactions < e.singlePeerTransactionThreshold {
		// First check if the peer is also a validator. If so, select the committing peer since it's more efficient, i.e. no Gossip required.
		for _, peer := range committers {
			if peer.HasRole(roles.ValidatorRole) {
				selectedValidator = peer

				logger.Infof("[%s] Selected validator is %s (committer,validator) since the number of transactions in block %d is %d which is less than the single validator threshold %d",
					e.channelID, selectedValidator.Endpoint, block.Header.Number, numTransactions, e.singlePeerTransactionThreshold)

				break
			}
		}

		if selectedValidator == nil && len(validators) > 0 {
			...
			logger.Infof("[%s] Selected validator is %s since the number of transactions in block %d is %d which is less than the single validator threshold %d",
				e.channelID, selectedValidator.Endpoint, block.Header.Number, numTransactions, e.singlePeerTransactionThreshold)
		}
	}

	if selectedValidator == nil && len(validators) == 0 {
		logger.Infof("[%s] There are no validators for block %d. Selecting the committer to validate the block.", e.channelID, block.Header.Number)

		for _, member := range committers {
			selectedValidator = member

			logger.Infof("[%s] Selecting committer %s to validate block %d", e.channelID, selectedValidator, block.Header.Number)

			break
		}

		if selectedValidator == nil {
			logger.Errorf("[%s] There are no validators or committers for block %d. Please ensure that at least one peer has the committer role.", e.channelID, block.Header.Number)
			...
			if selectedValidator != nil {
				if endpoint == selectedValidator.Endpoint {
					logger.Infof("[%s] Including peer %s since it is the selected validator for block %d (transactions: %d)",
						e.channelID, endpoint, block.Header.Number, numTransactions)

					return true
				}

				logger.Infof("[%s] Not including peer %s since the selected validator for block %d (transactions: %d) is %s",
					e.channelID, endpoint, block.Header.Number, numTransactions, selectedValidator.Endpoint)

				return false
			}

			if validators.ContainsPeer(endpoint) {
				logger.Infof("[%s] Including peer %s to validate block %d (transactions: %d)", e.channelID, endpoint, block.Header.Number, numTransactions)

				return true
			}

			logger.Infof("[%s] Not including peer %s to validate block %d (transactions: %d)", e.channelID, endpoint, block.Header.Number, numTransactions)
