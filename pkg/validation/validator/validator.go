...
func (v *validator) GetValidatingPeers(block *common.Block) ([]*discovery.Member, error) {
...
func (v *validator) validateRemaining(ctx context.Context, block *common.Block, notValidated map[int]struct{}) {
	logger.Debugf("[%s] Starting validation of %d transactions in block %d that were not validated ...", v.channelID, len(notValidated), block.Header.Number)
