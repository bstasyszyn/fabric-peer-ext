...
func New(gossip gossipProvider, txnProvider txnServiceProvider, dcasClientProvider dcasClientProvider, qep qeProvider) *AuthFilter {
	f := &AuthFilter{
...
func (f *AuthFilter) getPrivateDataNoLock(channelID, ccName string, args [][]byte) pb.Response {
	...
	qe, err := f.qeProvider.QueryExecutorProviderForChannel(channelID).NewQueryExecutorNoLock()
	if err != nil {
		return shim.Error(fmt.Sprintf("Failed to create DCAS client: %s", err))
	}
